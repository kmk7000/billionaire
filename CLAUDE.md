...
28. **스와이프 뒤로가기 (2026-08)** — `src/hooks/useSwipeBack.ts`. iOS의 네이티브 스와이프백은 **webview 히스토리**를 움직이는데 이 앱의 오버레이는 React 상태라 pop할 히스토리가 없다(웹 빌드에선 SPA 자체를 벗어난다). 그래서 직접 구현했다.
    ...
    - **아직 스와이프가 없는 곳**: 프로필 편집 모달 12종의 `isEditOpen`(スキル/職種/言語/ウェブサイト/受賞/資格/学歴/講演/出版/個人情報/経歴 편집/記事 편집) — 각 에디터 훅에 dirty 신호가 없어서(`usePersonalInfoEditor`만 `isUnchanged` 보유) 가드를 붙이려면 훅 11개에 스냅샷 비교를 먼저 넣어야 한다. 그 외 검색/날짜/학교 등 **피커성 오버레이**와 `MeishiScannerModal`(카메라)은 의도적으로 제외.
