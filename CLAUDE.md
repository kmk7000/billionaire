...
28. **스와이프 뒤로가기 (2026-08)** — `src/hooks/useSwipeBack.ts`. iOS의 네이티브 스와이프백은 **webview 히스토리**를 움직이는데 이 앱의 오버레이는 React 상태라 pop할 히스토리가 없다(웹 빌드에선 SPA 자체를 벗어난다). 그래서 직접 구현했다.
    ...
    - **편집 폼에는 일부러 넣지 않았다.** `MeishiEditView`와 프로필 편집 모달들은 스와이프 한 번에 입력 중이던 내용이 **말없이 사라진다.** 뒤로가기 버튼도 확인 없이 닫긴 하지만, 스와이프는 실수로 발동하기가 훨씬 쉽다. 넣으려면 미저장 변경 확인 다이얼로그가 먼저다.
    - 적용된 곳: `MeishiDetailView` / `ProfileOverlay` / `PostDetailOverlay` / `SettingsSubPage`(설정 하위 페이지 전체) / `SearchOverlay` / `NotificationsPanel` / `CallerIdInfoSheet`.
    - 검증(합성 터치 이벤트): 가장자리 스와이프→닫힘, 화면 중앙 시작→유지, 대각선/세로→유지, 짧은 스와이프→유지, 왼쪽 방향→유지.
