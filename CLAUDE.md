...
27. **`pt-safe`를 스크롤 컨테이너에 걸면 안 된다 — 헤더에 걸 것 (2026-08)** — 명함 상세/마이프로필에서 **내용이 상단바(뒤로가기 버튼 줄)를 뚫고 상태바 영역에 겹쳐 보이는** 문제. 오버레이 16개가 전부 같은 패턴이었다: `fixed inset-0 … overflow-y-auto pt-safe` + 내부에 `sticky top-0` 헤더.
    - **원인**: `padding-top`은 스크롤 영역의 **일부**라서 스크롤하면 같이 사라진다. 그리고 sticky는 scrollport(=padding box) 기준이므로 헤더는 **패딩 아래(y=59)**에 멈추는데, 그 위 `0~59px` 구간은 아무도 덮지 않는 빈 땅이 된다. 콘텐츠가 그 구간으로 올라오면 상태바에 그대로 비친다.
    - 브라우저에서 59px 인셋을 시뮬레이션해 실측: **옛 구조**는 scrollTop=120에서 이름(`h2`)이 `nameTop=7`로 상태바에 **42px 침범**, 헤더는 y=59에 정지. **새 구조**는 헤더가 y=0에 정지하고 스윕한 모든 스크롤 위치에서 침범 **0px**.
    - **해결**: `pt-safe`를 스크롤 컨테이너에서 떼어 **`sticky top-0` 헤더 자신에게** 옮긴다. 헤더가 인셋만큼 두꺼워지고 불투명 배경이 상태바 구간까지 덮으며, 콘텐츠는 그 아래로 지나간다. 16개 컨테이너 → 16개 헤더로 1:1 이동.
    - **주의**: `flex flex-col` 컨테이너(스크롤이 내부 자식에 있는 구조 — `SettingsSubPage` 등)의 `pt-safe`는 **문제가 없다.** 컨테이너 자체가 스크롤되지 않으므로 패딩이 사라지지 않는다. 옮기지 말 것. 판별 기준은 "그 엘리먼트에 `overflow-y-auto`가 같이 붙어 있는가".
    - App.tsx에서 한 번 과하게 적용했다가 되돌렸다 — 모바일 탭 헤더 3개는 이미 `<div class="lg:hidden pt-safe">` 안에 있어 이중 패딩이 되고, 데스크톱 헤더는 노치가 없다.
28. **스와이프 뒤로가기 (2026-08)** — `src/hooks/useSwipeBack.ts`. iOS의 네이티브 스와이프백은 **webview 히스토리**를 움직이는데 이 앱의 오버레이는 React 상태라 pop할 히스토리가 없다(웹 빌드에선 SPA 자체를 벗어난다). 그래서 직접 구현했다.
    - **가로 스크롤 영역과의 충돌이 핵심 난점.** 이 앱엔 명함첩 탭 줄·커뮤니티 칩바처럼 가로 스크롤이 여럿이다. 그래서 ①왼쪽 가장자리 **28px 안**에서 시작 ②시작 지점 조상 중 **아직 왼쪽으로 더 갈 수 있는** 가로 스크롤러가 있으면 양보 ③가로 이동이 세로보다 커야 함 ④멀티터치는 포기 — 네 조건을 모두 만족해야 발동한다.
    - **오버레이가 중첩되므로 최상단 하나만 반응해야 한다.** 리스너가 `window`에 붙으므로 그냥 두면 열려 있는 오버레이가 전부 같은 스와이프를 받아 한꺼번에 닫힌다. 모듈 레벨 스택에 등록하고 **맨 위 것만** 핸들러를 실행한다. 검증: 마이프로필 위에 ヘルプ를 띄우고 스와이프 → ヘルプ만 닫히고 아래는 유지.
    - **편집 폼은 미저장 확인 다이얼로그를 거친다 (2026-08 추가)** — `useUnsavedChangesGuard`(`src/hooks/`)가 **뒤로가기 버튼과 스와이프를 같은 `requestClose`로 묶는다.** 둘이 따로 놀 수 없고, 어느 쪽도 입력 중이던 내용을 말없이 버리지 않는다. 안 건드린 폼은 그냥 닫힌다(손대지도 않은 폼에 확인창을 띄우는 건 탭 한 번 더일 뿐).
      - 다이얼로그는 `src/components/ConfirmDialog.tsx`로 공용화했다(원래 `MeishiDetailView` 안에 인라인으로 있던 것). 문구는 `UNSAVED_CHANGES_DIALOG` 상수로 통일.
      - dirty 판정은 **연 시점의 값과 비교**한다. 되돌려 입력하면 clean으로 쳐서 확인창이 안 뜬다.
      - 다이얼로그가 떠 있는 동안은 스와이프를 정지시킨다(`enabled && !isPrompting`) — 확인창 아래에서 또 뒤로가기가 예약되면 안 된다.
      - 적용: `MeishiEditView` / `パスワード変更` / `パスワード再設定` / `1:1 お問い合わせ` / 프로필 `紹介`.
    - 적용된 곳(스와이프): `MeishiDetailView` / `MeishiEditView`(가드) / `ProfileOverlay` / `PostDetailOverlay` / `SettingsSubPage`(설정 하위 페이지 전체) / `SearchOverlay` / `NotificationsPanel` / `CallerIdInfoSheet` / `もっと見る` / `設定` / `アカウント管理` / `退会` / `パスワード変更`(가드) / `パスワード再設定`(가드) / `お問い合わせ`(가드) / `紹介`(가드) / `MeishiMapView` / `SimpleLoginSettings` / 経歴·記事 리스트.
    - **`SettingsModals`처럼 한 컴포넌트가 오버레이 여러 개를 가지면 z-순서 오름차순으로 등록할 것.** 스택은 **마지막 등록**에게 스와이프를 보내는데, 設定는 もっと見る를 닫지 않고 그 위에 열리므로 등록 순서가 곧 우선순위가 된다.
    - **프로필 편집 모달 12종까지 완료 (2026-08)** — 훅 11개에 dirty 신호를 넣어 마무리했다(`usePersonalInfoEditor`는 이미 `isUnchanged` 보유).
      - `src/hooks/useDirtySnapshot.ts` — **연 시점의 값을 baseline으로 잡고 현재 값과 비교**한다. "입력 이벤트가 한 번이라도 발생했는가"로 판정하지 않는 이유는, 되돌려 입력한 경우 clean으로 쳐야 사용자가 기대하는 동작이기 때문이다(실측 확인: 타이핑 후 원래 값으로 되돌리면 확인창이 안 뜬다).
      - 에디터마다 `open()`이 다르므로 baseline 잡는 위치가 다르다: ①프로필에서 시딩(スキル/職種/ウェブサイト/受賞/資格) ②항상 빈 폼(講演/出版 → `!!title || !!date`로 충분, 스냅샷 불필요) ③신규/기존 둘 다(記事/言語/経歴 → `openNew`와 `openExisting` 양쪽에서 capture) ④필드가 열기 전부터 유지됨(学歴 → `open()` 시점의 현재 값을 그대로 baseline으로).
      - `経歴`·`学歴`처럼 필드가 많은 폼은 **`formSnapshot()` 헬퍼 하나**를 두고 capture와 비교가 같은 목록을 쓰게 했다 — 따로 나열하면 필드 추가 시 한쪽만 빠뜨린다.
      - `CareerModals`/`ArticleModals`는 리스트와 편집 폼 **둘 다** 등록한다. 편집 폼이 리스트 위에 열리므로 **리스트 다음에** 등록해야 한다(스택은 마지막 등록에게 보낸다).
    - **최종 커버리지: 전체 오버레이 38개 중 32개.** 남은 6개는 의도적 제외 — 검색/날짜/학교/전공/학위/언어 선택 등 **피커성 오버레이**(페이지가 아니라 선택창)와 `MeishiScannerModal`(카메라 촬영 중 스와이프로 닫히면 곤란).
    - 검증(합성 터치 이벤트): 가장자리 스와이프→닫힘, 화면 중앙 시작→유지, 대각선/세로→유지, 짧은 스와이프→유지, 왼쪽 방향→유지. 중첩: もっと見る 위에 設定 → 스와이프 1회에 設定만 닫히고, 다시 스와이프하면 もっと見る가 닫힘. 가드: clean 폼→확인창 없이 닫힘, dirty 폼→확인창, 「編集を続ける」→유지, 「破棄する」→닫힘.
