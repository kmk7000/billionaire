...
export const DesktopSidebar: React.FC<{ onNavigateCommunity: () => void }> = ({ onNavigateCommunity }) => (
  <aside className="hidden lg:block w-80 shrink-0 space-y-4 sticky top-20">
    ...
    <div className="bg-surface p-4">
      <div className="flex items-center justify-between mb-3 pb-2 border-b border-line">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-sm text-ink">リアルタイム人気記事 Top 5</h3>
        </div>
      ...
      <div className="space-y-3">
        {[
          ...
          >
            <span className={`font-black text-xs w-4 text-center ${item.rank <= 3 ? 'text-accent' : 'text-ink-faint'}`}>
              {item.rank}
            </span>
    ...
    <div className="bg-surface p-4">
      <h3 className="font-bold text-sm text-ink mb-3">おすすめビジネスネットワーク</h3>
      <div className="space-y-3">
        {[
          ...
          <div key={idx} className="flex items-center justify-between text-xs">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-full bg-primary-soft border border-line flex items-center justify-center font-bold text-ink-muted">
                {prof.name[0]}
              </div>
              <div>
                <p className="font-bold text-ink">{prof.name}</p>
                <p className="text-[10px] text-ink-muted truncate max-w-[130px]">{prof.company}</p>
              </div>
            </div>
            <button className="px-2.5 py-1 bg-primary-soft text-primary hover:bg-primary hover:text-white transition-colors rounded-md text-[11px] font-bold">
              + Connect
            </button>
          </div>
        ))}
